...
#[must_use]
pub fn mse_loss(pred: &DiffTensor<f64>, target: &DiffTensor<f64>) -> f64 {
    let diff = &pred.data - &target.data;
    diff.iter().map(Complex::norm_sqr).sum::<f64>() / diff.len() as f64
}

/// Gradient of [`mse_loss`] with respect to `pred`.
#[must_use]
pub fn mse_loss_backward(pred: &DiffTensor<f64>, target: &DiffTensor<f64>) -> DiffTensor<f64> {
    let scale = 2.0 / pred.data.len() as f64;
    DiffTensor::from_array((&pred.data - &target.data) * scale)
}
...
/// `L = (1/N) * sum_i (|pred_i| - |target_i|)^2`.
#[must_use]
pub fn magnitude_mse_loss(pred: &DiffTensor<f64>, target: &DiffTensor<f64>) -> f64 {
    pred.data
        .iter()
