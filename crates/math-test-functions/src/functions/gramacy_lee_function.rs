...
mod tests {
    ...
    #[test]
    fn test_gramacy_lee_function_known_properties() {
        ...
        let meta = metadata
            .get("gramacy_lee_function")
            .expect("Function gramacy_lee_function should have metadata");
        ...
        for (minimum_coords, expected_value) in &meta.global_minima {
            ...
            let actual_value = gramacy_lee_function(&x);
        ...
        if !meta.global_minima.is_empty() {
            ...
            let result = gramacy_lee_function(&x);
